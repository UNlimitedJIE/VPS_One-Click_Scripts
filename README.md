...
维护菜单中的防火墙相关项目支持查看当前监听端口和 nftables 状态，也支持在 `VPS EXTRA PORTS` 受控区块中打开或关闭 TCP/UDP 入站端口。该能力只管理入站放行规则，不提供 DNAT、SNAT、MASQUERADE、端口中转或 NAT 转发；如果云厂商还有安全组或云防火墙，也需要在云控制台同步调整。
