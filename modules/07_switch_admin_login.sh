...
main() {
  ...
  if is_true "${PLAN_ONLY:-false}" || is_true "${DRY_RUN:-false}"; then
    log info "[plan] 正式执行时会先检查管理用户、公钥、sshd 语法、SSH 端口监听和 safe gate，再确认关闭 root 远程登录。"
    confirm_cutover_execution || return $?
    log info "[plan] would set PermitRootLogin=no, PubkeyAuthentication=yes, PasswordAuthentication=no, KbdInteractiveAuthentication=no."
    return 0
  fi
