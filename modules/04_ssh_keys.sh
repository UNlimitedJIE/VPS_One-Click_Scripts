...
main() {
  ...
  if ! id -u "${ADMIN_USER}" >/dev/null 2>&1; then
    if is_true "${PLAN_ONLY:-false}" || is_true "${DRY_RUN:-false}"; then
      log info "Plan/Dry-run note: ${ADMIN_USER} does not exist yet; assume /home/${ADMIN_USER} for SSH key preview."
    else
  ...
  if [[ -z "${home_dir}" ]] && { is_true "${PLAN_ONLY:-false}" || is_true "${DRY_RUN:-false}"; }; then
